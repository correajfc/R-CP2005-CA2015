...
OSMap+
  ...
  labs(title ="Zona Urbana de Santiago de Cali", x = "Long", y="Lat",caption="Datos en grados decimales, sistema de coordenas WGS83\nDatos: IDESC")
...
```{r mapas-poblacion-deciles, fig.width=5,fig.height=8,fig.cap="Mapas de las variables de población seleccionadas (en deciles).", fig.pos = 'p', out.height="90%"}
su.f %>% dplyr::select(-area_su)  %>%
  ...
  labs(caption = , "Fuente:CP2005 [@ca2015cali]")
...
```{r mapas-usopredios-cont, fig.asp=1,fig.cap="Mapas de las variables sobre el tipo de uso de los predios como porcentaje de la cantidad de predios (escala contínua)"}
...
- Mostrar una correlación fuerte (mayor a 0.6 se considera una asociación fuerte)
...
Para garantizar que las variables no están correlacionadas entre sí, se usaron el coeficiente de correlación de Pearson, usado para detectar relaciones lineales, y el coeficiente de Spearman para detectar relaciones en que exhiben relaciones no lineales.
