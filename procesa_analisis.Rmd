...
```{r mapas-lisa-copa-wq,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ de `sqrt.area_copa`"}

pintar_mapa_su_LISA_var(regresion.arboles,"sqrt.area_copa",W_queen, wname = "W_q",nrow =1)

```
En los estos mapas de LISA de ambas matrices se observan que los sectores en rojo (H-H) y azul (L-L) identifac los lugares con autocorrelacion positiva, foemando grupos. No se presentan valores negativos de autocorrelación.   

```{r mapas-lisa-copa-wd,fig.asp=0.618,fig.cap="Mapas LISA para la matriz $W_d$ de `sqrt.area_copa`"}
...
Para la variable dependiente porcentaje de cobertura de copa los resulados del analisis de autocorrelacieon se presentan a continuación.

```{r code-moran-copaap-w, echo=TRUE}
moran.test(lm.best.cobertura.ap$model$sqrt.cobertura_copa.ap.mxn, listw=W_queen)
moran.test(lm.best.cobertura.ap$model$sqrt.cobertura_copa.ap.mxn, listw=W_dist1000.inv, zero.policy = TRUE)
...
moran.plot(lm.best.cobertura.ap$model$sqrt.cobertura_copa.ap.mxn, 
             listw=W_queen, 
             
             pch=16, col="grey50",
             cex=.5, quiet=T, 
             labels=as.character(regresion.arboles$SETU_CCDGO),
             xlab="sqrt.cobertura_copa.ap", 
             ylab="sqrt.cobertura_copa.ap (Spatial Lag)", main="Gráfico de Moran Wq") 
moran.plot(lm.best.cobertura.ap$model$sqrt.cobertura_copa.ap.mxn, 
             listw=W_dist1000.inv, 
             zero.policy = TRUE,
             pch=16, col="grey50",
             ...
             labels=as.character(regresion.arboles$SETU_CCDGO),
             xlab="sqrt.cobertura_copa.ap", 
             ylab="sqrt.cobertura_copa.ap (Spatial Lag)", main="Gráfico de Moran Wd") 

```


La matrix $W_q$ capatura un poco mejor de forma global la autocorrelacieon del porcentaje de cobertura de copa; esta versón relativa del area de copa tiene una autocorrelación espacial positiva mucho más fuerte y significativa, produciendo *clusters* de sectores urbanos más grandes.  Los mapas LISA muestran los focos de esta autocorrelación usando la matriz para $W_q$ (figura \@ref(fig:mapas-lisa-copaap-wq)) y para $W_d$ (figura \@ref(fig:mapas-lisa-copaap-wd)) . No se presentan valores negativos de autocorrelación. 

```{r mapas-lisa-copaap-wq,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ de `sqrt.cobertura_copa.ap`"}

pintar_mapa_su_LISA_var(regresion.arboles,"sqrt.cobertura_copa.ap",W_queen, wname = "W_q",nrow =1)

```

```{r mapas-lisa-copaap-wd,fig.asp=0.618,fig.cap="Mapas LISA para la matriz $W_d$ de `sqrt.cobertura_copa.ap`"}

pintar_mapa_su_LISA_var(regresion.arboles,"sqrt.cobertura_copa.ap",W_dist1000.inv, wname = "W_d", nrow =1)
...
El mismo análisis se aplica al modelo de porcentaje de cobertura de copa (`` `r str_c("sqrt.cobertura_copa.ap.mxn",str_c(names(lm.best.cobertura.ap$model)[-1],collapse =" + "), sep= " ~ ") ` ``). 
...
Para ambos matrices el valor de Moran Global es mayor que 0 y significativo, el doble más alto que para el modelo de área de copa; el indicador de porcentual y el residuo del modelo para ajustarlo muestra una autocorrelación espacial en lugares levemente desplazados de los encontrados con el area de copa. En las figuras \@ref(fig:mapas-lisa-rescopaap-wq) y \@ref(fig:mapas-lisa-rescopaap-wd) observamos que los grupos de sectores son más pequeños que los de las variables dependientes, lo que podría deberse a que las variables independientes presentan un patron espacial similar, y que por consiguiente, al introducir estos retardos al modelo van a ayudar a absorver esa diferencias para mejorar la estimación. 
...
Los resultados nos invitan a proceder a realizar un ajuste en ambos modelos introduciendo algún tipo de estructura espacial. Posiblemente la matriz $W_d$ sea la candidata, pues parece capturar mejor en los test de autocorrelación global la existencia de grupos formados por fenomenos de dispersión o derrame en la distribución de la copa y de las variables independientes elegidas para modelarlas, pues se reduce la autocorrelación al calcularla para los residuos.
...
Una alternativa metodológica es probar los 3 tipos de modelos con ambas matrices $W_d$ que resultó capturar mejor la asociacion espacial en ls datos y comparar sus resultados.
...
sem.mod.sqrt.area_copa.wq<-errorsarlm(formula = as.formula(lm.mod.area_copa),
                                      data = lm.mod.area_copa$model,
                                      listw = W_queen, zero.policy = T,tol.solve=1.0e-30)
sem.sm<-summary(sem.mod.sqrt.area_copa.wq,Nagelkerke=T)
sem.sm
#SD 
...
sd.sm<-summary(sd.mod.sqrt.area_copa.wq,Nagelkerke=T)
sd.sm
...
slx.sm<-summary(slx.mod.sqrt.area_copa.wq)
slx.sm
...
gmt.sar<-moran.test(sar.mod.sqrt.area_copa.wq$residuals,W_queen,zero.policy = T)
bpt.sar<-bptest.sarlm(sar.mod.sqrt.area_copa.wq)
swt.sar<-shapiro.test(sar.mod.sqrt.area_copa.wq$residuals)
akaike.sar<-AIC(sar.mod.sqrt.area_copa.wq)
adjRsrt.sar<-sar.sm$NK
mediares.sar<-mean(sar.sm$residuals)
mse.sar<-mean(sar.sm$residuals^2)  
lglk.sar<-logLik(sar.mod.sqrt.area_copa.wq)

gmt.sd<-moran.test(sd.mod.sqrt.area_copa.wq$residuals,W_queen,zero.policy = T)
bpt.sd<-bptest.sarlm(sd.mod.sqrt.area_copa.wq)
swt.sd<-shapiro.test(sd.mod.sqrt.area_copa.wq$residuals)
akaike.sd<-AIC(sd.mod.sqrt.area_copa.wq)
adjRsrt.sd<-sd.sm$NK
mediares.sd<-mean(sd.sm$residuals)
mse.sd<-mean(sd.sm$residuals^2)  
lglk.sd<-logLik(sd.mod.sqrt.area_copa.wq)

gmt.sem<-moran.test(sem.mod.sqrt.area_copa.wq$residuals,W_queen,zero.policy = T)
bpt.sem<-bptest.sarlm(sem.mod.sqrt.area_copa.wq)
swt.sem<-shapiro.test(sem.mod.sqrt.area_copa.wq$residuals)
akaike.sem<-AIC(sem.mod.sqrt.area_copa.wq)
adjRsrt.sem<-sem.sm$NK
mediares.sem<-mean(sem.sm$residuals)
mse.sem<-mean(sem.sm$residuals^2) 
lglk.sem<-logLik(sem.mod.sqrt.area_copa.wq)

gmt.slx<-moran.test(slx.mod.sqrt.area_copa.wq$residuals,W_queen,zero.policy = T)
bpt.slx<-lmtest::bptest(slx.mod.sqrt.area_copa.wq)
swt.slx<-shapiro.test(slx.mod.sqrt.area_copa.wq$residuals)
akaike.slx<-AIC(slx.mod.sqrt.area_copa.wq)
adjRsrt.slx<-slx.sm$adj.r.squared
mediares.slx<-mean(slx.sm$residuals)
mse.slx<-mean(slx.sm$residuals^2)  
lglk.slx<-logLik(slx.mod.sqrt.area_copa.wq)

gmt.lm<-moran.test(lm.mod.area_copa$residuals,W_queen,zero.policy = T)
bpt.lm<-lmtest::bptest(lm.mod.area_copa)
swt.lm<-shapiro.test(lm.mod.area_copa$residuals)
akaike.lm<-AIC(lm.mod.area_copa)
lm.sm<-summary(lm.mod.area_copa)
adjRsrt.lm<-lm.sm$adj.r.squared
mediares.lm<-mean(lm.sm$residuals)
mse.lm<-mean(lm.sm$residuals^2)  
lglk.lm<-logLik(lm.mod.area_copa)

modelos<-c("Medidas","OLS","SLX","SAR","SEM","SD")
medidasfit<-c("Globla Moran'I",
              ...
              "AIC",
              "Log likelihood")
ols<-c(gmt.lm$estimate[1],gmt.lm$p.value,
       swt.lm$statistic,swt.lm$p.value,
       bpt.lm$statistic,bpt.lm$p.value,
       mediares.lm,
       mse.lm ,
       adjRsrt.lm,
       NA,
       akaike.lm,
       lglk.lm)
slx<-c(gmt.slx$estimate[1],gmt.slx$p.value,
       swt.slx$statistic,swt.slx$p.value,
       bpt.slx$statistic,bpt.slx$p.value,
       mediares.slx,
       mse.slx ,
       adjRsrt.slx,
       NA,
       akaike.slx,
       lglk.slx)
sar<-c(gmt.sar$estimate[1],gmt.sar$p.value,
       swt.sar$statistic,swt.sar$p.value,
       bpt.sar$statistic,bpt.sar$p.value,
       mediares.sar,
       mse.sar ,
       NA,
       adjRsrt.sar,
...
sd<-c(gmt.sd$estimate[1],gmt.sd$p.value,
       swt.sd$statistic,swt.sd$p.value,
       bpt.sd$statistic,bpt.sd$p.value,
       mediares.sd,
       mse.sd ,
       NA,
       adjRsrt.sd,
...
sem<-c(gmt.sem$estimate[1],gmt.sem$p.value,
       swt.sem$statistic,swt.sem$p.value,
       bpt.sem$statistic,bpt.sem$p.value,
       mediares.sem,
       mse.sem ,
       NA,
       adjRsrt.sem,
...
tablatem<-data_frame(medidasfit,ols,slx,sar,sem,sd)
knitr::kable(tablatem, digits = 5,col.names = modelos, caption = "Metricas de ajuste para los modelos de área de copa")


```


Al comparar los resultados de las métricas de ajuste se identifica al modelo SD o mixto(variables con retardo y autocorrelación) con el mejor AIC. De hecho la estimación de SLX y SD tiene el mismo valor de AIC, pero el modelo SD ha logrado eliminar la autocorrelación espacial global en los residuos, al igual que SAR y SEM (ver figura \@ref(fig:moranplot-resmodel-all-wq)). Aunque persiste la no normalidad de los residuos y la heterocedasticidad como los muestran los test y las gráficas diagnosticar, el error cometido disminuye y los coeficientes pueden considerarse más confiables. El $\rho$ y $\lambda$ de las estimaciones con términos de autoregresivos es significativo, lo que significa que la inclusión de los retardos funciona correctamente, aunque solo en los coeficientes de la variable retardada `cuarto.porcentaje` es significativo, pero esta no es significativa como variable sin retardo, sugiriendo que interesa sobre efecto sobre el área de copa en los sectores vecinos a los focos de mayor concentración de viviendas tipo cuarto, pero no es importante en el resto de sectores. Como era de esperarse el porcentaje de área de espacios verdes es significativo en las variables sin retardo, pero no genera ningún efecto de derrame en la cobertura de copa de los sectores vecinos.
...
moran.plot(sem.mod.sqrt.area_copa.wq$residuals, 
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos sar", 
           ylab="residuos sar (Spatial Lag)", main="Gráfico de Moran SEM") 
...
moran.plot(slx.mod.sqrt.area_copa.wq$residuals, 
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos sar", 
           ylab="residuos sar (Spatial Lag)", main="Gráfico de Moran SLX") 
...
moran.plot(sd.mod.sqrt.area_copa.wq$residuals, 
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos sar", 
           ylab="residuos sar (Spatial Lag)", main="Gráfico de Moran SD") 
```
...
resmodelos<-regresion.arboles %>% select(SETU_CCDGO) %>% 
  ...
  mutate(sd.resid =sd.mod.sqrt.area_copa.wq$residuals )
...
gmt.sar<-moran.test(sar.cobertura.ap.wq$residuals,W_queen,zero.policy = T)
bpt.sar<-bptest.sarlm(sar.cobertura.ap.wq)
swt.sar<-shapiro.test(sar.cobertura.ap.wq$residuals)
akaike.sar<-AIC(sar.cobertura.ap.wq)
adjRsrt.sar<-sar.sm$NK
mediares.sar<-mean(sar.sm$residuals)
mse.sar<-mean(sar.sm$residuals^2)  
lglk.sar<-logLik(sar.cobertura.ap.wq)

gmt.sd<-moran.test(sd.cobertura.ap.wq$residuals,W_queen,zero.policy = T)
bpt.sd<-bptest.sarlm(sd.cobertura.ap.wq)
swt.sd<-shapiro.test(sd.cobertura.ap.wq$residuals)
akaike.sd<-AIC(sd.cobertura.ap.wq)
adjRsrt.sd<-sd.sm$NK
mediares.sd<-mean(sd.sm$residuals)
mse.sd<-mean(sd.sm$residuals^2)  
lglk.sd<-logLik(sd.cobertura.ap.wq)

gmt.sem<-moran.test(sem.cobertura.ap.wq$residuals,W_queen,zero.policy = T)
bpt.sem<-bptest.sarlm(sem.cobertura.ap.wq)
swt.sem<-shapiro.test(sem.cobertura.ap.wq$residuals)
akaike.sem<-AIC(sem.cobertura.ap.wq)
adjRsrt.sem<-sem.sm$NK
mediares.sem<-mean(sem.sm$residuals)
mse.sem<-mean(sem.sm$residuals^2) 
lglk.sem<-logLik(sem.cobertura.ap.wq)

gmt.slx<-moran.test(slx.cobertura.ap.wq$residuals,W_queen,zero.policy = T)
bpt.slx<-lmtest::bptest(slx.cobertura.ap.wq)
swt.slx<-shapiro.test(slx.cobertura.ap.wq$residuals)
akaike.slx<-AIC(slx.cobertura.ap.wq)
adjRsrt.slx<-slx.sm$adj.r.squared
mediares.slx<-mean(slx.sm$residuals)
mse.slx<-mean(slx.sm$residuals^2)  
lglk.slx<-logLik(slx.mod.sqrt.area_copa.wq)

gmt.lm<-moran.test(lm.best.cobertura.ap$residuals,W_queen,zero.policy = T)
bpt.lm<-lmtest::bptest(lm.best.cobertura.ap)
swt.lm<-shapiro.test(lm.best.cobertura.ap$residuals)
akaike.lm<-AIC(lm.best.cobertura.ap)
lm.sm<-summary(lm.best.cobertura.ap)
adjRsrt.lm<-lm.sm$adj.r.squared
mediares.lm<-mean(lm.sm$residuals)
mse.lm<-mean(lm.sm$residuals^2)  
lglk.lm<-logLik(lm.mod.area_copa)
...
tablatem<-data_frame(medidasfit,ols,slx,sar,sem,sd)
knitr::kable(tablatem, digits = 5,col.names = modelos, caption = "Metricas de ajuste para los modelos de porcentaje de área de copa")


```


Al comparar los resultados de las métricas de ajuste se identifica al modelo   
...
```{r mapa-resid-copaap-modelo-all}

resmodelos<-regresion.arboles %>% select(SETU_CCDGO) %>% 
  mutate(SETU_CCDGO = as.character(SETU_CCDGO))%>%
  mutate(ols.resid=lm.best.cobertura.ap$residuals) %>%
  mutate(slx.resid =slx.cobertura.ap.wq$residuals)%>%
  mutate(sar.resid =sar.cobertura.ap.wq$residuals)%>%
  mutate(sem.resid =sem.cobertura.ap.wq$residuals)%>%
  mutate(sd.resid =sd.cobertura.ap.wq$residuals )
