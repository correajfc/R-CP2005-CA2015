...
ggpairs(
  regresion.arboles[,indep.poblacion.abs] %>% 
    reemplzar_nombres_df(df_nombres_variables), 
  lower = list(continuous = wrap_fn_with_params(lowerFn, method = "lm")),
...
ggpairs(
  regresion.arboles[,indep.poblacion.percent]%>% 
    reemplzar_nombres_df(df_nombres_variables), 
  lower = list(continuous = wrap_fn_with_params(lowerFn, method = "lm")),
  diag = list(continuous = wrap_fn_with_params("barDiag", colour = "white",fill ="steelblue")),
  upper = list(continuous = wrap_fn_with_params("cor", size = 4))#,
  # title = "Comparación por pares entre predictores de población porcentuales y \nla densidad de población"
)+ theme_grey(base_size = 8)+
  theme(strip.text = element_text(size = 5))
...
- Para el área de copa (`area_copa`) los predictores seleccionados son: `` `r str_c(indep.poblacion.copa.sel%>% reemplzar_nombres_vector(df_nombres_variables), collapse = ", ")` ``. 

- Para la cobertura de copa (`cobertura_copa.ap`) los predictores seleccionados son: `` `r str_c(indep.poblacion.copa.ap.sel %>% reemplzar_nombres_vector(df_nombres_variables), collapse = ", ") ` `` 
...
```{r tile-poblacion-spearman, fig.asp=1,fig.cap="Coeficiente Spearman entre variables de población"}
pintar_corrmatrix(reemplzar_nombres_df(regresion.arboles,df_nombres_variables),
...
```{r tile-copa-poblacion-pearson, fig.asp=0.67,fig.cap="Coeficiente Pearson entre cobertura de copa y variables de población"}
...
```{r tile-copa-poblacion-spearman, fig.asp=0.67,fig.cap="Coeficiente Spearman entre cobertura de copa y variables de población"}
pintar_corrmatrix_XY(reemplzar_nombres_df(regresion.arboles,df_nombres_variables),
                     x=reemplzar_nombres_vector(indep.poblacion,df_nombres_variables), 
                     y=reemplzar_nombres_vector(dependientes.arboles,df_nombres_variables), 
                     method_cor = "spearman")#+  
...
Los resultados de los test Shapiro-Wilk indican no normalidad en los residuos en ambos modelos, heterocedasticidad como muestra el test Breusch-Pagan y posibles no linealidades como se observa en las gráficas diagnósticas de la regresión de ambos modelos( ver figuras \@ref(fig:diagn-mod-best-lm-copa) y \@ref(fig:diagn-mod-best-lm-copaap)). 

Sin embargo, el ajuste de ambos modelos tiene media de los residuos muy cercanas a 0, al igual que el error cuadrático medio (MSE). En el caso del área de copa se obtiene un *adjR-square* de `r summary(lm.mod.area_copa) %>% broom::glance() %>% .$adj.r.squared %>% scales::percent() %>% str_c(collapse = ", ")`, que es aceptable. Las variables significativas para el área de copa son:  `r c(indep.poblacion.copa.sel,indep.predios.copa.sel)[c(1,2,5,6)] %>% reemplzar_nombres_vector(df_nombres_variables)%>% backtick() %>% str_c(collapse = ", ")`. Los resultados confirman que al nivel de toda el área de estudio, las condiciones de acceso a la educación de la población, la densidad de población, el tipo de vivienda y la disponibilidad de EV se correlaciona con el acceso a servicios ambientales del AU.

Para la cobertura de copa la única variable significativa es `r indep.poblacion.copa.ap.sel[1] %>% reemplzar_nombres_vector(df_nombres_variables)%>% backtick()`. Este único indicador porcentual, que hace una descripción local y comparable entre los SU, explica el `r summary(lm.mod.cobertura.ap) %>% broom::glance() %>% .$adj.r.squared %>% scales::percent()` de la variabilidad de los datos, reforzando la importancia del indicador de acceso a educación superior como predictor del acceso a servicios ambientales del AU.

Para la siguientes fases se ignoraron las variables no significativas de los modelos lineales. 

```{r best-model-au, eval=TRUE, echo=FALSE, results='hide'}
...
```{r coef-lm-copa}
nombres_coef <- summary(lm.area_copa)$coefficients %>% row.names() %>% 
  str_remove_all(".mxn") %>% 
  # str_replace_all("[\\._]"," ") %>% 
...
t1 %>% 
  bind_cols(terminos = nombres_coef,.) %>% 
knitr::kable(format = formato_render,  
             booktabs = T, 
             digits=3, 
             caption = "Coeficientes OLS de área de copa - Log(AC)" ,  col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)")) %>% 
...
nombres_coef <- summary(lm.mod.cobertura.ap)$coefficients %>% 
  row.names() %>% 
  str_remove_all(".mxn") %>% 
  # str_replace_all("[\\._]"," ") %>% 
  reemplzar_nombres_vector(df_nombres_variables) %>% 
...
kableExtra::kable(format = formato_render,  
                  # booktab = TRUE,
                  digits=5, 
                  caption = "Coeficientes OLS de cobertura de copa - (CC)" , 
                  booktabs = TRUE, 
...
Es clara la similitud entre los grupos obtenidos para una misma variable dependiente con ambos diseños de matriz $W$ (figura \@ref(fig:mapas-lisa-copa-wq) y figura \@ref(fig:mapas-lisa-copa-wd) para el área de copa y \@ref(fig:mapas-lisa-copaap-wq) y \@ref(fig:mapas-lisa-copaap-wd) para la cobertura de copa), posiblemente porque no existen diferencias notables entre los dos tipos de estructura de vecindad propuestas (figura \@ref(fig:ws-su-reg)). 
...
data_frame(metrica =nom_estimado_moran, 
           log_area =mt,
           cobertura =mt2) %>% 
knitr::kable(format = formato_render,  booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5,escape = F,
             caption = "Test de Moran - Área de copa para $W_q$ y $W_d$" ) %>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5, escape = F,
             caption = "Test de Moran - Porcentaje de cobertura de copa para $W_q$ y $W_d$" ) %>% 
  kable_styling(latex_options = "HOLD_position") 
...
```{r mapas-lisa-copa-wq,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ del indicador área de copa"}
...
```{r mapas-lisa-copa-wd,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_d$ del indicador área de copa"}
...
```{r mapas-lisa-copaap-wq,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ del indicador cobertura de copa"}
...
```{r mapas-lisa-copaap-wd,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_d$ de del indicador cobertura de copa"}
...
La tabla \@ref(tab:moran-rescopa-w) muestra ambos diseños de matriz $W$ presentan un valor de Moran Global mayor que 0 y significativo para los residuos del OLS de área de copa, al igual que para los residuos del OLS del porcentaje de cobertura (ver tabla \@ref(tab:moran-rescopaap-w)).  
...
knitr::kable(format = formato_render,  booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5, escape = F,
             caption = "Test de Moran - Residuos de OLS Área de copa para $W_q$ y $W_d$" ) %>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5, escape = F,
             caption = "Test de Moran - Residuos de OLS Porcentaje de cobertura de copa para $W_q$ y $W_d$" ) %>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de área de copa $W_q$") %>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de área de copa con $W_d$") %>% 
  kable_styling(latex_options = "HOLD_position") 
...
```{r diag-model-espaciales, fig.asp=1, fig.cap="Diagnóstico comparativo entre modelos espaciales de área de copa"}
...
t2<-rbind(c("$\\lambda$","Likelihood ratio","p-valor"),
          r1)
knitr::kable(format = formato_render,  booktabs = T,
             escape = F,
             unname(t2), digits=3,caption = 'Coeficiente de autocorrelación modelo SEM de área de copa $W_d$')%>% 
  kable_styling(latex_options = "HOLD_position") 
...
# knitr::kable(format = formato_render, escape = F,  booktabs = T,t1, digits=3,caption = 'Coeficientes del modelo SD de área de copa $W_q$')

nombres_coef <- summary(sd.mod.log.area_copa.wq)$Coef %>% row.names() %>% 
  str_remove_all(".mxn") %>% 
  reemplzar_nombres_vector(df_nombres_variables) %>%
  # str_replace_all("[\\._]"," ") %>% 
  str_replace("\\(Intercept\\)","Intercepto")
t1<-summary(sd.mod.log.area_copa.wq)$Coef %>% as_data_frame()
t1 %>% bind_cols(terminos = nombres_coef,.) %>% 
kableExtra::kable(format = formato_render,  
                  # booktab = TRUE,
                  digits=3, 
                  caption = "Coeficientes del modelo SD de área de copa $W_q$" , 
                  booktabs = TRUE, 
col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)"))%>% 
  kable_styling(latex_options = "HOLD_position") 
...
t2<-rbind(c("$\\rho$","Likelihood ratio","p-valor"),
          r1)
knitr::kable(format = formato_render, escape = F, booktabs = T,unname(t2), digits=3,caption = 'Coeficiente de autocorrelación modelo SD de área de copa $W_q$')%>% 
  kable_styling(latex_options = "HOLD_position") 
...
tabla<-fitstats_laglm_df(lm.best.cobertura.ap,W_queen) %>% 
  ...
  rename(`SD`=fitstats)
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de porcentaje de área de copa $W_q$")%>% 
  kable_styling(latex_options = "HOLD_position") 
...
tabla<-fitstats_laglm_df(lm.best.cobertura.ap,W_dist1000.inv) %>% 
  ...
  rename(`SD`=fitstats)
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de porcentaje de área de copa $W_d$")%>% 
  kable_styling(latex_options = "HOLD_position") 
...
diagPltsSD.wd<-diagPlotlaglm(sd.cobertura.ap.wd,theme.base_size = 7)
diagPltsOLS<-diagPlotlaglm(lm.best.cobertura.ap,theme.base_size = 7)
...
t2<-rbind(c("$\\rho$","Likelihood ratio","p-valor"),
          r1)
knitr::kable(format = formato_render, escape = F, booktabs = T,unname(t2), digits=3,caption = 'Coeficiente de autocorrelación modelo SD $W_d$ de porcentaje de área de copa ')%>% 
  kable_styling(latex_options = "HOLD_position") 
...
# knitr::kable(format = formato_render,  booktabs = T,t1, digits=3,caption = 'Coeficientes del modelo SD de porcentaje de área de copa $W_d$')

nombres_coef <- summary(sd.cobertura.ap.wd)$Coef %>% 
  row.names() %>% 
  str_remove_all(".mxn") %>%
  reemplzar_nombres_vector(df_nombres_variables) %>%
  # str_replace_all("[\\._]"," ") %>% 
...
t1<-summary(sd.cobertura.ap.wd)$Coef %>% as_data_frame()
t1 %>% bind_cols(terminos = nombres_coef,.) %>% 
kableExtra::kable(format = formato_render,  
                  # booktab = TRUE,
                  digits=3, 
                  caption = "Coeficientes del modelo SD de porcentaje de área de copa $W_d$" , 
                  booktabs = TRUE, 
col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)"))%>% 
  kable_styling(latex_options = "HOLD_position") 
...
En el caso de los EV la literatura ofrece variedad de medidas sobre acceso en relación con la distancia o con el área disponible. Se eligieron dos métricas: el porcentaje de área de espacio verde de un sector censal, para cuantificar beneficios a nivel local; y la razón área disponible entre distancia (`ia.areas.dist`) (ecuación \@ref(eq:areas-dists)) que expresa el acceso más allá de los límites del SU. El valor es cercano a cero cuando el área disponible es cercana a cero o cuando la distancia es mucho mayor que el área media de espacio verde en el radio de búsqueda, limitado a 1000 $m$, un orden de magnitud menor que el área media de EV (`r regresion.EV$area_ep %>% mean() %>% scales::comma(accuracy = 0.1)` $m^2$). La distancia promedio del centroide de un SU al conjunto de EV es `r ia.1000.n %>% mean(na.rm = T) %>%  round(digits = 3) %>% scales::comma(accuracy = 0.1)` $m$. Como se observa en la figura \@ref(fig:mapa-dependienteEV-sel), esta medición parece un versión interpolada del indicador local de acceso porcentaje de área de EV (`area_ep.porcentaje`), y hace evidente un patrón espacial de grupos con mejor acceso a EV, no uniforme ni aleatorio. 
...
```{r mapa-dependienteEV-sel, fig.asp= 0.7,out.width="80%",fig.cap="Métricas de acceso a espacio verdes seleccionadas"}
...
El último bloque de variables indaga sobre las áreas y proporciones de las manzanas de cada sector censal y la vocación como pública o privada de los espacios dentro de un sector urbano. Las figuras \@ref(fig:tile-ev-fisica-pearson) y \@ref(fig:tile-ev-fisica-spearman) muestran que el área media de las manzanas (`área media de manzana`) de los sectores urbanos se relaciona de forma positiva con ambos índices de acceso, mucho más fuertemente que las variables poblacionales y de uso de predios. Aunque parece haber una fuerte correlación de los indicadores de acceso con las áreas privadas, públicas y del sector urbano, estas hacen parte de los cálculos que generan estos índices, produciendo en efecto ficticio en la correlación, razón por la que se incluyen en la modelación.
...
- Para el `r reemplzar_nombres_vector("area_ep.porcentaje",df_nombres_variables)` los predictores seleccionados son `r str_c(c(indep.poblacion.area_ep.porcentaje.sel, indep.predios_estruct.area_ep.ptje.sel)%>% reemplzar_nombres_vector(df_nombres_variables)%>% backtick(), collapse = ", ")`. 

- Para `r reemplzar_nombres_vector("ia.areas.dist",df_nombres_variables)`  los predictores seleccionados son `r str_c(c(indep.poblacion.ia.areas.dist.sel,indep.predios_estruct.areas.dist.sel) %>%reemplzar_nombres_vector(df_nombres_variables) %>%  backtick(), collapse = ", ")`.  
...
Los resultados de los test Shapiro-Wilk indican no normalidad en los residuos en ambos modelos, heterocedasticidad como muestra el test Breusch-Pagan y posibles no linealidades como se observa en las gráficas diagnósticas de la regresión de ambos modelos (ver figuras \@ref(fig:diagn-lm-areaptj-sel) y \@ref(fig:diagn-lm-areadist-sel)). El nivel explicativo de la variabilidad de los datos de ambos modelos es bajo. Sin embargo, el ajuste de ambos modelos tiene media de los residuos muy cercanas a 0, al igual que el error cuadrático medio (MSE). 


Las variables significativas para el `r reemplzar_nombres_vector("area_ep.porcentaje",df_nombres_variables)` son `r c(indep.poblacion.area_ep.porcentaje.sel, indep.predios_estruct.area_ep.ptje.sel)[c(2,3)] %>% reemplzar_nombres_vector(df_nombres_variables) %>%backtick() %>% str_c(collapse = ", ")`. Estos resultados muestran que no existe evidencia significativa de que el acceso a EV en un SU esté relacionado con variables étnicas, de discapacidad o de acceso a la educación. Sin embargo es muy importante la relación positiva con aspectos estructurales representados por el área media de las manzanas y una relación negativa con el porcentaje de viviendas tipo cuarto, estas últimas concentradas en la zona centro y de ladera del área urbana de Santiago de Cali (ver figura \@ref(fig:mapas-usopredios-cont)).


Para la relación de área distancia se confirma la significancia de `r indep.predios_estruct.areas.dist.sel[2] %>% reemplzar_nombres_vector(df_nombres_variables) %>% backtick() %>% str_c(collapse = ", ")` y, en ausencia de la variable `vivienda tipo cuarto [%]`, se incorpora `uso de unidad económica [%]` que exhibe un patrón espacial similar. A diferencia del índice local de acceso, en `razón área-distancia` se evidenció una relación negativa con el porcentaje de personas con alguna limitación.
...
nombres_coef <- summary(lm.area_ep.ptje)$coefficients %>% row.names() %>% str_remove_all(".mxn") %>% reemplzar_nombres_vector(df_nombres_variables) %>% str_replace("\\(Intercept\\)","Intercepto")
t1<-summary(lm.area_ep.ptje)$coefficients %>% as_data_frame()
t1 %>% bind_cols(terminos = nombres_coef,.) %>% 
knitr::kable(format = formato_render,  booktabs = T, digits=5, caption = "Coeficientes OLS de Porcentaje de EV" ,  col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)"))%>% 
  kable_styling(latex_options = "HOLD_position") 
...
nombres_coef <- summary(lm.ia.areas.dist)$coefficients %>% row.names() %>% str_remove_all(".mxn") %>% reemplzar_nombres_vector(df_nombres_variables) %>% str_replace("\\(Intercept\\)","Intercepto")
t1<-summary(lm.ia.areas.dist)$coefficients %>% as_data_frame()
t1 %>% bind_cols(terminos = nombres_coef,.) %>% 
kable(format = formato_render,  
             booktabs = T, digits=5,
             caption = "Coeficientes OLS de áreas-distancia", 
             col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)")
             )%>% 
  kable_styling(latex_options = "HOLD_position") 
...
fitstats_lm_df(lm.area_ep.ptje.sel) %>% 
  rename(EV=fitstats) %>%
   left_join(fitstats_lm_df(lm.ia.areas.dist.sel),by= "medidasfit") %>%
  rename( `Área-Distancia`=fitstats) %>%  
kable( format = formato_render,  booktabs = TRUE,
      caption = "Resumen métricas de ajuste OLS Indice contenedor (EV) y de acceso área-distancia ", digits = 5)%>% 
  kable_styling(latex_options = "HOLD_position") 
...
Los mapas LISA para ambos indicadores de acceso a EV usando la matriz $W_q$ muestran los grupos de sectores autocorrelacionados (figuras \@ref(fig:mapas-lisa-areaep-wq) y \@ref(fig:mapas-lisa-areadist-wq)). Los grupos formados muestran agrupaciones de alto acceso a EV en relación al resto de la ciudad.
Se aprecia que se forman *clusters* alrededor de cuatro zonas en el caso del porcentaje de área de EV y dos para el indicador de relación áreas-distancia, coincidentes con el anterior. Esos sectores albergan equipamientos de ciudad como un cementerio de gran tamaño, las universidades y zonas conservadas de riberas de ríos. El grupo que se forma al oriente de la ciudad es donde se encuentra la laguna del Pondaje. 
...
data_frame(metrica =nom_estimado_moran, 
           log_area =mt,
           cobertura =mt2) %>% 
knitr::kable(format = formato_render,escape = F,
             booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5,
             caption = "Test de Moran - Porcenatje de EV para $W_q$ y $W_d$" )%>% 
  kable_styling(latex_options = "HOLD_position") 
...
data_frame(metrica =nom_estimado_moran, 
           ...
           cobertura =mt2) %>% 
knitr::kable(format = formato_render, escape = F,
             booktabs = T,col.names = c(" ","$W_{q}$","$W_{d}$"),
             digits=5,
             caption = "Test de Moran - Razón área distancia para $W_{q}$ y $W_{d}$" )%>% 
  kable_styling(latex_options = "HOLD_position") 
...
```{r mapas-lisa-areaep-wq, fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ de área de EV [\\%]" }
...
```{r mapas-lisa-areadist-wq,fig.asp=0.618, fig.cap="Mapas LISA para la matriz $W_q$ de razón área-distancia" }
...
La tabla \@ref(tab:moran-resareaep-w) muestra que ambos diseños de matriz $W$ presentan un valor de Moran Global mayor que 0 y significativo para los residuos del OLS del porcentaje de área de EV, al igual que para los residuos del OLS del indicador áreas-distancia (ver tabla \@ref(tab:moran-resareadist-w)).  
...
knitr::kable(format = formato_render, escape = F,
             booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5,
             caption = "Test de Moran - Residuos de OLS Porcentaje EV $W_q$ y $W_d$" )%>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render, escape = F,
             booktabs = T,col.names = c(" ","$W_q$","$W_d$"),
             digits=5,
             caption = "Test de Moran - Residuos de OLS relación áreas distancias para $W_q$ y $W_d$" )%>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de porcentaje de área de EV con $W_q$")%>% 
  kable_styling(latex_options = "HOLD_position") 
...
t2<-rbind(c("$\\lambda$","Likelihood ratio","p-valor"),
          r1)
knitr::kable(format = formato_render, escape = F, booktabs = T,unname(t2), digits=3,caption = 'Coeficiente de autocorrelación modelo SEM de porcentaje de área de EV')%>% 
  kable_styling(latex_options = "HOLD_position") 
...
knitr::kable(format = formato_render,  booktabs = T,tabla, digits = 5, caption = "Metricas de ajuste para los modelos de áreas-distancia de EV")%>% 
  kable_styling(latex_options = "HOLD_position") 
...
nombres_coef <- summary(sd.areas.dist)$Coef %>% row.names() %>% str_remove_all(".mxn") %>% reemplzar_nombres_vector(df_nombres_variables)%>% str_replace("\\(Intercept\\)","Intercepto")
t1<-summary(sd.areas.dist)$Coef %>% as_data_frame()
t1 %>% bind_cols(terminos = nombres_coef,.) %>% 
kableExtra::kable(format = formato_render,  
                  # booktab = TRUE,
                  digits=3, 
                  caption =  'Coeficientes del modelo SD índice de relación área distancia de EV' , 
                  booktabs = TRUE, 
col.names = c("Término","Estimado","Error std.","t-valor","Pr(>|t|)"))%>% 
  kable_styling(latex_options = "HOLD_position") 
...
t2<-rbind(c("$\\rho$","Likelihood ratio","p-valor"),
          r1)
knitr::kable(format = formato_render, escape = F, booktabs = T,unname(t2), digits=3,caption = 'Coeficiente de autocorrelación modelo SD índice de relación área distancia de EV')%>% 
  kable_styling(latex_options = "HOLD_position") 
...
```{r diag-model-areasdist-espaciales, fig.asp=0.6, fig.cap="Diagnóstico comparativo entre modelos espaciales del indicador área-distancia"}
diagPltsSAR<-diagPlotlaglm(sar.areas.dist,theme.base_size = 7)
# diagPltsSEM<-diagPlotlaglm(sem.areas.dist)
# diagPltsSD<-diagPlotlaglm(sd.areas.dist)
# diagPltsSLX<-diagPlotlaglm(slx.areas.dist)
diagPltsOLS<-diagPlotlaglm(lm.ia.areas.dist.sel,theme.base_size = 7)
