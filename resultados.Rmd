...
#### Área de copa
```{r code-lm-areacopa, eval=TRUE, echo=TRUE}
...
```{r code-lm-cobertura, eval=TRUE, echo=TRUE}
# coberbtura AP
...
lm.morantest(lm.best.cobertura.ap, 
             W_queen, alternative="two.sided",
             zero.policy = T)
lm.morantest(lm.best.cobertura.ap, 
             W_dist1000.inv, 
...
A continuación se presentan los resultados del modelado de los 4 tipos de modelos geoestadísticos para el área de copa.
...
Los resultados muestran que las variables `cuarto.porcentaje`  `area_media_manzana` son significativas.

```{r code-lm-area_ep-sel,echo=FALSE,eval=TRUE}
...
moran.test(regresion.EV$area_ep.porcentaje, 
...
moran.test(regresion.EV$ia.areas.dist, 
           listw=W_dist1000.inv.ev,
           zero.policy = TRUE)
moran.test(regresion.EV$ia.areas.dist, 
           listw=W_queen.ev)
...
lm.morantest(lm.area_ep.ptje.sel, 
             W_queen.ev, 
             alternative="two.sided",
             zero.policy = T)
lm.morantest(lm.area_ep.ptje.sel, 
...
lm.morantest(lm.ia.areas.dist.sel, 
             W_queen.ev, 
             alternative="two.sided",
             zero.policy = T)
lm.morantest(lm.ia.areas.dist.sel, 
