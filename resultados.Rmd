...
Lo relevante en ambos diseños y los resultados de los análisis es que la vencidad de los SU exponen un grado de autocorrelación espacial y por lo tanto los grupos de alta y baja prestación en los servicios ecosistémicos del AU configuran evidencia de los sesgos espaciales en la distribución de dichos beneficios, rechanzado la hipotesis nula de distribucion espacial aleatoria o uniforme.
...
Ambos diseños de matriz $W$ presentan un valor de Moran Global mayor que 0 y significativo. $W_d$ lo hace mejor en los residuos, mientras $W_q$ lo hace mejor con la variable dependiente .  


al introducir estos retardos al modelo van a ayudar a absorber esa diferencias para mejorar la estimación. 
...
A continuación se presentan los resultados del modelado de los 3 tipos de modelos que usan las matrices $W_d$ y $W_q$ geoestadísticos para el área de copa. Las tablas \@ref() y \@ref() resumen las métricas de ajuste de los modelos de área de copa con ambas matrices. 

Al comparar los resultados de las métricas de ajuste se identifica al modelo SD o mixto(variables con retardo y autocorrelación) con el mejor AIC. 

El modelo SD ha logrado eliminar la autocorrelación espacial global en los residuos, al igual que SAR y SEM (ver figura \@ref(fig:moranplot-resmodel-all-wq)). Aunque persiste la no normalidad de los residuos y la heterocedasticidad como los muestran los test y las gráficas diagnósticas, el error cometido disminuye y los coeficientes pueden considerarse más confiables. El $\rho$ y $\lambda$ de las estimaciones con términos de autorregresivos es significativo, lo que implica que la inclusión de los retardos funciona correctamente, aunque solo en los coeficientes de la variable retardada `cuarto.porcentaje` es significativo, aunque esta no tiene un coeficiente significativo como variable sin retardo, sugiriendo que interesa su efecto sobre el área de copa en los sectores vecinos a los focos de mayor concentración de viviendas tipo cuarto, pero no es importante en el resto de sectores. Como era de esperarse el porcentaje de área de espacios verdes es significativo en las variables sin retardo, pero no genera ningún efecto de derrame en la cobertura de copa de los sectores vecinos. A pesar de que los test de normalidad y heterocedasticidad no son exitosos, las gráficas diagnósticas muestran que los problemas se presentan en los valores extremos. 
...
```{r moranplot-resmodel-all-wq, fig.cap="Gráfico de Moran para los residuos de los mejores modelos espaciales de área de copa$", fig.show="hold", out.width="49%"}
moran.plot(lm.best.area_copa$residuals, 
           ...
           xlab="residuos", 
           ylab="residuos sar (Spatial Lag)", main="Gráfico de Moran OLS Wq")
moran.plot(lm.best.area_copa$residuals, 
           listw=W_dist1000.inv, 
           zero.policy = TRUE,
           pch=16, col="black",
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos", 
           ylab="residuos sar (Spatial Lag)", main="Gráfico de Moran OLS Wd")

moran.plot(sem.mod.log.area_copa.wd$residuals, 
           listw=W_dist1000.inv, 
           zero.policy = TRUE,
           pch=16, col="black",
           cex=.5, quiet=T, 
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos", 
           ylab="residuos (retardo espacial)", main="Gráfico de Moran SEM Wd") 
...
moran.plot(sd.mod.log.area_copa.wq$residuals, 
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos", 
           ylab="residuos (retardo espacial)", main="Gráfico de Moran SD Wq") 
```


```{r diag-model-espaciales, fig.asp=1.4, fig.cap="Diagnóstico comparativo entre modelos modelos espaciales de área de copa"}
...
```{r moranplot-resmodel-all-copaap-wq, fig.cap="Gráfico de Moran para los residuos de los mejores modelos espaciales del porcentaje de área de copa", fig.show="hold", out.width="49%"}
...
moran.plot(sar.cobertura.ap.wq$residuals, 
           ...
           labels=as.character(regresion.arboles$SETU_CCDGO),
           xlab="residuos sd", 
           ylab="residuos sd (Spatial Lag)", main="Gráfico de Moran SAR Wq") 
